...
# Function to simulate mouse drag using Quartz
def quartz_mouse_drag(start_x, start_y, end_x, end_y, duration=5):
    # Move the mouse to the start position
    event = CGEventCreateMouseEvent(None, kCGEventMouseMoved, (start_x, start_y), 0)
    ...
    # Simulate dragging the mouse to the end position
    steps = int(duration * 60)  # Smooth movement over the duration
    for i in range(steps):
        intermediate_x = start_x + (end_x - start_x) * i / steps
        intermediate_y = start_y + (end_y - start_y) * i / steps
        event = CGEventCreateMouseEvent(None, kCGEventLeftMouseDragged, (intermediate_x, intermediate_y), 0)
        CGEventPost(kCGHIDEventTap, event)
        time.sleep(1 / 60)  # 60 FPS smooth dragging
...
def screenshot_mode():
    ...
    try:
        # Trigger macOS screenshot tool (or any other method for screenshot capture)
        pyautogui.hotkey("command", "shift", "4")
        print("Command + Shift + 4 triggered for macOS screenshot tool")

        # Wait for the screenshot tool to be active
        time.sleep(1)

        # Simulate dragging the mouse from top-left to bottom-right
        print(f"Dragging mouse from {top_left} to {bottom_right}")
        quartz_mouse_drag(top_left[0], top_left[1], bottom_right[0], bottom_right[1], duration=5)
...
try:
    while True:
        ...
        if landmark_points:
            ...
            if (left[0].y - left[1].y) > 0.03 and (right[0].y - right[1].y) > 0.03:
